...
- Launching a job now hands back how to supervise it, because launching supervises nothing and assembling a watch invocation by hand is where the config path gets guessed and the timeout invented. `agent_start_job` and `agent_plan_dispatch` return a `supervision` block with a ready-to-run `watch_command` covering every job just started, timed at the longest job's own `timeout_sec` plus a finalization slack (`FINALIZATION_SLACK_SEC`) so the watch outlives the controller gate battery; `supervised` is `true` only when a `wait` actually returned a settled verdict. The CLI's `start` prints the same command to stderr when nothing is supervising, leaving stdout JSON pipeable. `agent_watch_events` responses that are not `done` now carry `snapshot_only` and `watch_command`: a single pass reads like a subscription otherwise, and an agent that called it once, reported "watcher is up" and moved on left five workers unobserved on 2026-08-13.
- Added `agent_watch_events`, the MCP equivalent of `agent-control watch --events`, so a coordinator without a shell no longer has to hand-roll a status poll loop. It takes one **non-blocking** pass over the same selection the CLI accepts (`job_ids`, `plan_id`, `task_glob`) and returns `{events, cursor, done, pending, terminal_statuses}`; passing the returned `cursor` back yields only what changed, so a job that outlives a single call is followable without holding one open — the previous surface (`agent_watch_job`, capped at the 300 s `_MAX_LONG_POLL_SEC`) needed up to a dozen sequential blocking calls to see one 60-minute job through. The server stays stateless: `WatchEventStream.export_cursor`/`load_cursor` round-trip the per-job dedup state (last status, settled, last stale heartbeat) through the caller instead of keeping a stream instance alive per selection, and cursor entries for jobs that left the selection are dropped so a stale cursor cannot keep a watch pending forever. `done` uses the settled predicate the CLI exit code is built on, and `pending` reports each unsettled job's last status so a status that never goes terminal on its own (`cancel_requested`) is visible rather than silently waited on.
- Added `agent_terminal_statuses` (MCP mirror of `agent-control statuses`), returning the authoritative terminal-status vocabulary, the finalization-settled values, the event kinds and the cursor version. `terminal_statuses` also ships in every `agent_watch_events` response. Watchers that retype this list get it wrong — an omitted `partial`, `worker_error` or `inefficient_tool_usage` turns a finished job into an indefinite wait.
...
- Added a `timed_out` controller quality gate report status, distinct from `failed`: a failed gate always wins the roll-up, but when nothing failed and at least one gate ran out of its `timeout_sec`, the report now reads `timed_out` instead of collapsing into `failed`. Both still block `review_ready` identically — `docs/operations.md` invariants on missing/failed/timed-out/drifted/uncovered evidence are unchanged — but the verification bundle's new `review_blocked_reason` (also surfaced in `inbox list`'s `verification_summary`) names which one happened and which gates were involved, so a reviewer no longer has to open `native-quality.json` to tell "we did not find out" from "we found a problem".
- Added `[control.defaults] native_quality_global_max_parallel` (default `4`) and `NativeQualityGateSlotBroker`, a cross-process bound on concurrently running controller quality gate processes. `native_quality_max_parallel` only ever bounded gates *within* one job's battery (an in-process `ThreadPoolExecutor`), so N jobs finalizing at once could still put `N * max_parallel` gate processes — several of them full test-suite runs — on the box at the same time. The new broker reuses the same cross-process SQLite lease pattern as the Codex quota broker (`GlobalQuotaBroker`), since finalization can happen in separate processes and an in-process semaphore would not see leases held elsewhere. A gate that cannot obtain a slot before its own `timeout_sec` elapses is recorded as `timed_out` rather than run past its budget or bypass the bound.
)
)
)
